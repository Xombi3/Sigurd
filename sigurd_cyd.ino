...
// ═══════════════════════════════════════════════════════════
//  SERIAL DEBUG — set to 0 to disable all serial output
// ═══════════════════════════════════════════════════════════
#define SIGURD_DEBUG 1

#if SIGURD_DEBUG
  #define DBG(msg)           Serial.println(msg)
  #define DBGf(fmt, ...)     Serial.printf(fmt "\n", ##__VA_ARGS__)
  #define DBG_SECTION(name)  Serial.printf("\n=== %s ===\n", name)
#else
  #define DBG(msg)
  #define DBGf(fmt, ...)
  #define DBG_SECTION(name)
#endif
...
SPIClass            sdSPI(HSPI);  // SD on HSPI — keeps VSPI free for touch
// Touch pins (VSPI)
...
// Touch calibration (PorkChop CYD values)
#define T_XMIN 340
#define T_XMAX 3800
#define T_YMIN 250
#define T_YMAX 3750
...
void IRAM_ATTR snifferCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  deauthCount++;
  Serial.printf("[DEAUTH] type=0x%02X count=%d\n", frameType, deauthCount);
  if (deauthLogCount < DEAUTH_LOG_MAX) {
    ...
    statScore  += 5; statLevel = 1 + (statScore / 100);
  }
}

void startDeauthDetection() {
  // Promiscuous mode works alongside station mode — no disconnect needed
  esp_wifi_set_promiscuous(true);
  esp_wifi_set_promiscuous_rx_cb(snifferCallback);
  deauthRunning = true;
  strncpy(comment, "Deauth detector active.", 95);
  sdLog("# Deauth detection started");
  DBG("Deauth detection started");
}
...
void runWifiScan() {
  ...
  DBG_SECTION("WIFI SCAN");
  int n=WiFi.scanNetworks(false,true);
  DBGf("Scan complete — %d networks found", n);
  char b[64];
  if(n>0){
    statTargets+=n; statScore+=n*5; statLevel=1+(statScore/100);
    snprintf(b,sizeof(b),"Found %d network(s)! Valhalla calls.",n);
    strncpy(comment,b,95);
    // Log open networks to SD
    for(int i=0;i<n;i++){
      bool open=(WiFi.encryptionType(i)==WIFI_AUTH_OPEN);
      DBGf("  AP %d: %s  RSSI:%d  %s  BSSID:%s", i+1,
        WiFi.SSID(i).c_str(), WiFi.RSSI(i),
        open?"OPEN":"ENC", WiFi.BSSIDstr(i).c_str());
      snprintf(b,sizeof(b),"WIFI,%s,%d,%s",
        WiFi.SSID(i).c_str(),WiFi.RSSI(i),open?"OPEN":"ENC");
      sdLog(b);
      if(open){statCreds++;statScore+=20;statLevel=1+(statScore/100);}
    }
...
void runHostScan() {
  ...
  setLED(false,false,true); drawScreen();
  DBG_SECTION("HOST SCAN");
  DBGf("Scanning subnet: %s.%d-%d", SCAN_SUBNET, SCAN_START, SCAN_END);
  ...
  for(int i=SCAN_START;i<=SCAN_END&&hostCount<MAX_HOSTS;i++){
    ...
    animateTick();
    if(hostAlive(ip)){
      DBGf("HOST ALIVE: %s", ip);
      strncpy(hostList[hostCount],ip,15);
      // Resolve hostname
      String hn=resolveHostname(ip);
      DBGf("  Hostname: %s", hn.length()>0 ? hn.c_str() : "(none)");
  ...
  animState=ANIM_IDLE; statAttacks++;  // scan done
...
void triggerAlert(const char* msg) {
  ...
  statScore += 25;
  statLevel = 1 + (statScore / 100);  // already correct
  char logbuf[96];
...
String grabHttpTitle(const char* ip, uint16_t port) {
  WiFiClient c;
  c.setTimeout(600);
  DBGf("HTTP title grab: %s:%d", ip, port);
  if (!c.connect(ip, port)) { DBGf("  connect failed"); return ""; }
  c.printf("GET / HTTP/1.0\r\nHost: %s\r\nConnection: close\r\n\r\n", ip);
  unsigned long t = millis();
  String body = "";
  bool full = false;
  while (!full && millis() - t < 1500) {
    while (c.available() && !full) {
      body += (char)c.read();
      if (body.length() > 2048) full = true;
    }
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = i+1; j < n; j++) {
      if (WiFi.SSID(j) == ssidA) {
        ...
        if (macA != macB) {
          DBGf("EVIL TWIN DETECTED: %s on %s and %s",
            ssidA.substring(0,16).c_str(), macA.c_str(), macB.c_str());
          char alert[95];
...
void tickMonitor() {
  ...
  DBG_SECTION("MONITOR SWEEP");
  DBGf("Free heap: %d bytes", ESP.getFreeHeap());
...
void wifiConnect() {
  ...
  for(int i=0;i<20&&WiFi.status()!=WL_CONNECTED;i++){
    setLED(false,false,i%2); delay(500);
    DBGf("  attempt %d/20 status=%d", i+1, WiFi.status());
  }
  updateWifiInfo();
  if(WiFi.status()==WL_CONNECTED){
    ...
    statScore+=20; statLevel=1+(statScore/100);
    DBGf("WiFi OK — IP: %s RSSI: %d dBm", WiFi.localIP().toString().c_str(), WiFi.RSSI());
    DBG("Starting web server on port 80...");
    setupWebServer();
    DBG("Web server OK");
    char logbuf[64];
    ...
    DBG("WiFi FAIL — check SSID/PASS or signal strength");
  }
...
//  TOUCH
// ═══════════════════════════════════════════════════════════
// Touch state — debounce mirrors PorkChop CYD approach
...
bool getTouch(int& tx, int& ty) {
  ...
  DBGf("RAW: px=%d py=%d pz=%d", p.x, p.y, p.z);
  if (p.z < 100) return false;
  int sx = map(p.x, T_XMIN, T_XMAX, 0, SCREEN_W);
  int sy = map(p.y, T_YMIN, T_YMAX, 0, SCREEN_H);
  tx = constrain(sx, 0, SCREEN_W-1);
  ty = constrain(sy, 0, SCREEN_H-1);
  DBGf("TOUCH: x=%d y=%d z=%d", tx, ty, p.z);
  return true;
}
...
void setup() {
  ...
  DBG_SECTION("SIGURD CYD BOOT");
  DBGf("Free heap at boot: %d bytes", ESP.getFreeHeap());
  DBGf("CPU freq: %d MHz", ESP.getCpuFreqMHz());
  DBGf("Flash size: %d bytes", ESP.getFlashChipSize());
  ...
  tft.fillScreen(C_BG);
  DBG("TFT OK");
  delay(100);  // let HSPI settle before starting VSPI touch

  // Sprite — created once, never recreated
  sprite.createSprite(SPR_W,SPR_H);
  sprite.setColorDepth(16);
  DBGf("Sprite created: %dx%d", SPR_W, SPR_H);
  DBGf("Free heap after sprite: %d bytes", ESP.getFreeHeap());

  // Touch
  // SD card
  DBG("Touch init...");
  delay(50);
  touchSPI.begin(TOUCH_CLK_PIN, TOUCH_MISO_PIN, TOUCH_MOSI_PIN, TOUCH_CS_PIN);  // matches PorkChop exactly
  ts.begin(touchSPI);
  ts.setRotation(0);  // 2432S028R default
  DBG("Touch OK");

  DBG("SD init...");
  sdSPI.begin(14, 12, 13, SD_CS);  // HSPI: CLK=14 MISO=12 MOSI=13
  if(SD.begin(SD_CS, sdSPI)){
    sdAvailable=true;
    sdLog("# Sigurd CYD boot");
    DBGf("SD OK — card size: %llu MB", SD.cardSize() / (1024*1024));
  } else {
    sdAvailable=false;
    DBG("SD FAIL — no card or wiring issue");
  }
...
void loop() {
  ...
  // Heap monitor — print every 30s
  static unsigned long lastHeapMs = 0;
  if(millis()-lastHeapMs>30000){
    lastHeapMs=millis();
    DBGf("HEAP: free=%d min=%d", ESP.getFreeHeap(), ESP.getMinFreeHeap());
  }
  ...
  int tx,ty;
  delay(10);  // brief yield between touch polls
  // Check debounce BEFORE reading touch to avoid wasted SPI reads
  unsigned long now=millis();
  if(now-lastTouchMs<400) return;
  if(!getTouch(tx,ty)) return;
  lastTouchMs=now;
  DBGf("ZONE: tx=%d ty=%d  HDR<28 S1=28-78 S2=78-128 ACT=128-174 BOT=174+", tx, ty);

  // Header tap: toggle RSSI graph / monitor
  if(ty>=HDR_Y&&ty<HDR_H){ DBG("-> HDR");
    if(tx < SCREEN_W/2){
  ...
  // Stats row 1: WiFi | Host | Port scan (widened touch target 28-78)
  else if(ty>=28&&ty<78){ DBGf("-> S1 tx=%d",tx);
    int t=SCREEN_W/3;
    if(tx<t) runWifiScan();
    else if(tx<t*2) runHostScan();
    else runPortScan();
  }
  // Stats row 2: left=toggle deauth, right=port scan (widened 78-128)
  else if(ty>=78&&ty<128){ DBGf("-> S2 tx=%d",tx);
    int t=SCREEN_W/2;
    if(tx<t){
      ...
      // Partial redraw — just update action row, don't full redraw which can stall
      tft.fillRect(0, ACTION_Y, SCREEN_W, ACTION_H, C_BG);
      tft.setTextColor(C_FG,C_BG); tft.setTextSize(1);
      tft.drawString(currentAction, 34, ACTION_Y+4);
      char _wb[48]; snprintf(_wb,sizeof(_wb),"WiFi: %.18s",wifiName);
      tft.setTextColor(C_GREY,C_BG); tft.drawString(_wb,34,ACTION_Y+17);
      if(deauthRunning){ tft.setTextColor(C_RED,C_BG); tft.drawString("DEAUTH WATCH",34,ACTION_Y+28); }
      else             { tft.setTextColor(C_BG,C_BG);  tft.drawString("            ",34,ACTION_Y+28); }
    } else {
      runPortScan();
    }
  }
  // Action row + comment zone (128-174)
  else if(ty>=128&&ty<174){ DBG("-> ACTION");
    setRandomComment(POOL_IDLE, POOL_IDLE_N, 0);
    lastCommentMs = millis();
    animState=ANIM_TALK;
    drawComment();
  ...
  // Bottom panel (174+)
  else if(ty>=174){ DBGf("-> BOT tx=%d",tx);
    if(tx<SCREEN_W/2){
